...
open(30, file="./Reports/mixedlayer_report/Recent/profiles/" // filename //".csv")
open(31, file="./Reports/mixedlayer_report/Recent/profiles/" // filename // "PARAMETERS.csv")
...
if (seaprofile.eq.1) then
	...
	salinity(j) = 34 + (0.25 * j / (nint(d_max / (2 * d_grid))))
	temp(j) = (273.15 - 1.8)
	end do
...
elseif (seaprofile.eq.3) then
	do j = 1, nint(d_max / (2 * d_grid))
	salinity(j) = 34 + (0.75 * j / (nint(d_max / (2 * d_grid))))
	temp(j) = (273.15 - 1.8)
	end do
	do j = nint(d_max/(2*d_grid)) + 1, (nint(d_max / d_grid) + 1)
	salinity(j) = 34.75
...
elseif (seaprofile.eq.4) then
	do j = 1, nint(d_max / (2 * d_grid))
	salinity(j) = 34 + (1.0 * j / (nint(d_max / (2 * d_grid))))
	temp(j) = (273.15 - 1.8)
	end do
	do j = nint(d_max/(2*d_grid)) + 1, (nint(d_max / d_grid) + 1)
	salinity(j) = 35
...
!=========================================================================
	...
	write(30, 40)  seaprofile  ,d_mix_max,  s_mix_max, t_mix_max - 273.15
